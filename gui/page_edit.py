...
_ = Translator('page_edit', 'en').lang.gettext
...
class PageEditRaw(tk.Frame):
    ...
    def _build(self):
        self._frame_metadata_enrichment = tk.LabelFrame(self, text=_('Uppdatera metadata i råfiler'))
    ...
    def _build_metadata_enrichment(self):
        ...
        LISTBOX_TITLES = dict(title_items=dict(text=_('Ej valda serier'),
                                               fg='red',
                                               font='Helvetica 12 bold'),
                              title_selected=dict(text=_('Valda serier'),
                                                  fg='green',
                                                  font='Helvetica 12 bold'), )

        self._source_dir = components.DirectoryButtonText(frame,
                                                          'metadata_packs_source',
                                                          title=_('Välj serier från mapp'),
                                                          row=r,
                                                          column=c)
        r += 1
        prop = dict(
        ...
        self._lims_path = components.FilePathButtonText(frame_lims,
                                                            ...
                                                            title=_('Välj LIMS-fil'),
                                                            row=0,
                                                            column=1)
        tkw.grid_configure(frame_lims, nr_columns=2)

        r += 1
        tk.Button(frame, text=_('Uppdatera metadata'), command=self._update_metadata).grid(row=r, column=c, sticky='w')
    ...
    def _update_metadata(self, event=None):
        ...
        if self._boolvar_sharkweb.get():
            ...
            if not sharkweb_file_path:
                msg = _('Ingen sharkweb-fil vald')
                logger.warning(msg)
                messagebox.showwarning(_('Använd sharkweb-fil'), msg)
                return
        ...
        if self._boolvar_lims.get():
            ...
            if not lims_file_path:
                msg = _('Ingen LIMS-file vald')
                logger.warning(msg)
                messagebox.showwarning(_('Använd LIMS-fil'), msg)
                return
            logger.info(f'Updating metadata from {lims_file_path=}')

        output_dir = self._target_dir.get()
        if not output_dir:
            msg = _('Ingen mapp att spara till vald')
            logger.warning(msg)
            messagebox.showwarning(_('Uppdatera metadata'), msg)
            return

        packs = [self._all_packs[key] for key in self._packs_listbox.get_selected()]
        if not packs:
            msg = _('Inga filer valda')
            logger.warning(msg)
            messagebox.showwarning(_('Uppdatera metadata'), msg)
            return
        try:
            ...
            msg = _('Metadata har lagts till i {} profiler.').format(len(packs))
            logger.info(msg)
            messagebox.showinfo('Uppdatera metadata', msg)
        except Exception:
            msg = traceback.format_exc()
            logger.critical(msg)
            messagebox.showerror(_('Uppdatera metadata'), msg)
            raise
