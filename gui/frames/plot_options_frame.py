...
class PlotOptionsFrame(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(self, text='xmin').grid(row=r, column=1, **layout)
        tk.Label(self, text='xmax').grid(row=r, column=2, **layout)
        ...
        for key, par in self._config.items():
            ...
            ent_min.grid(row=r, column=1, **layout)
            ent_max = tk.Entry(self, textvariable=var_max)
            ent_max.grid(row=r, column=2, **layout)
